    ...
    static void addDrone(DroneDetails tempDrone) {
        ...
        */
        ...
        if (newDrone) {
            ...
            drones.add(drone);
            //###save drone to database
           java.sql.Connection cn_drone=null;
           try
           {
            Connect1 v=new Connect1();
            cn_drone=v.con;
            Statement stmt=cn_drone.createStatement();
            stmt.executeUpdate("insert into drone(id,name,xpos,ypos) values('"+tempDrone.getId()+"','"+tempDrone.getName()+"','"+tempDrone.getX_pos()+"','"+tempDrone.getY_pos()+"')");
            cn_drone.close();
           }
        catch(Exception e){
        ...
        */
        if (fires.isEmpty()) {
           ...
           try
           {
            ...
            cn_drone=v.con;
            Statement stmt=cn_drone.createStatement();
            stmt.executeUpdate("insert into fire(id,isActive,intensity,burningAreaRadius,xpos,ypos) values('','"+fireId+"','0','"+tempFire.getSeverity()+"','0','"+tempFire.getX_pos()+"','"+tempFire.getY_pos()+"')");
            cn_drone.close();
           }
